...
<#
.SYNOPSIS
    Start Echo Bot with automatic environment management
.DESCRIPTION
    This script ensures a working Python virtual environment exists and is properly
    configured before running the echo bot. It will create or repair the environment
...
.EXAMPLE
    .\start-echo-bot-new.ps1
.EXAMPLE
    .\start-echo-bot-new.ps1 -ConfigFile config.json -LogLevel DEBUG
#>
...
Write-Host "  Echo Bot (kryten-misc) Startup" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "Responds to '!echo <message>' in chat" -ForegroundColor Gray
...
try {
    ...
    Write-Host ""
    if ($exitCode -eq 0) {
        Write-Success "Echo bot stopped cleanly"
    } else {
        Write-ErrorMsg "Echo bot exited with error code: $exitCode"
    }
    
    exit $exitCode
} catch {
    Write-Host ""
    Write-ErrorMsg "Failed to execute echo bot: $_"
    Write-Info "Check echo_bot.log for details"
    exit 1
}
