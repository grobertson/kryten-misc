...
check_kryten_py() {
    ...
    if "$python_exe" -c "import kryten" &> /dev/null; then
        print_success "kryten-py package is installed"
        return 0
    else
        print_warning "kryten-py package not found"
        return 1
    fi
}
...
echo ""
echo "═══════════════════════════════════════════════════════"
echo "  Echo Bot (kryten-misc) Startup"
echo "═══════════════════════════════════════════════════════"
echo ""
...
if [[ ! -f "$CONFIG_PATH" ]]; then
    ...
    if [[ -f "$SCRIPT_DIR/config.example.json" ]]; then
        print_info "Example config available: config.example.json"
        print_info "Copy and customize it: cp config.example.json config.json"
    fi
    exit 1
fi
...
if ! check_kryten_py "$VENV_PYTHON"; then
    ...
    if ! check_kryten_py "$VENV_PYTHON"; then
        print_error "kryten-py still missing after installation"
        exit 1
    fi
fi
...
echo -e "\033[90mResponds to '!echo <message>' in chat\033[0m"
